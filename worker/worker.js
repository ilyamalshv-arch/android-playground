...
async function classify(body, env) {
  ...
  const lang = body.lang === "en" || body.lang === "es"
    ? "Текст может быть на английском или испанском; названия состояний — на языке приложения. " : "";
...
export default {
  async fetch(request, env) {
    ...
    const LANGS = {
      ...
      es: "\n\nIDIOMA: la persona usa la app en español de Argentina. Escribí toda la respuesta en castellano rioplatense, cálido y natural, con voseo (vos sentís, tenés, podés; nunca tú).",
    };
    const style = (STYLES[body.style] || STYLES.gentle) + (LANGS[body.lang] || "");
    const model = MODELS[body.model] || env.MODEL || DEFAULT_MODEL;
